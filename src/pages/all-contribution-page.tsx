...
import { ALL_CONTRIBUTION_USER_COUNT, CONTRIBUTION_ACTIVITIES } from '../mocks/contribution';
import type { ContributionActivity, ContributionProductType } from '../types/contribution';
...
function ActivityMessage({ activity }: { activity: ContributionActivity }) {
  const nickname = <span className='break-all font-semibold'>{activity.nickname}</span>;
  ...
  switch (activity.type) {
    ...
    case 'PRODUCT_REMAINING':
      return (
        <p>
          {nickname}님은 {activity.productName} 제작까지 <strong className='font-bold text-main-green2'>{activity.remainingCount}회</strong> 남았어요!
        </p>
      );
    case 'RECYCLING_COMPLETED':
      return (
        <p>
          {nickname}님이 오늘 <strong className='font-bold text-main-green2'>{activity.recyclingCount}번째</strong> 분리수거를 완료했어요
        </p>
      );
...
function ContributionActivityCard({ activity }: { activity: ContributionActivity }) {
  const isProductActivity = activity.type === 'PRODUCT_IN_PROGRESS' || activity.type === 'PRODUCT_REMAINING';
  const icon = isProductActivity && activity.productType ? productIcons[activity.productType] : countIcon;
  const iconAlt = isProductActivity ? `${activity.productName ?? '물품'} 아이콘` : '분리수거 아이콘';
