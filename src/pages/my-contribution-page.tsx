  ...
  return (
    ...
    <section>
        ...
        <h2 className='text-[20px] font-bold leading-[1.35] text-text'>
            ...
            <br />
            <span className='text-main-green2'>{achievedItemName}</span>
            를 만들었어요!
        </h2>
      </div>
      </section>
...
<section className='mx-4 mt-8 rounded-[24px] bg-white px-6 py-8 shadow-[0_8px_24px_rgba(0,0,0,0.08)]'>
  <div className='relative'>
    ...
    >
      <path
        d='M33.3 10 H79 Q83.3 10 83.3 14 V42 Q83.3 46 79 46 H16.7 H4 Q0 46 0 50 V78 Q0 82 4 82 H83.3'
        stroke='#06C65F'
        strokeWidth='2'
        strokeDasharray='5 6'
        strokeLinecap='round'
  ...
  <div className='relative z-10 grid grid-cols-6 gap-y-7'>
    {items.map((item, index) => (
      <div
        key={item.id}
        className={`col-span-2 flex flex-col items-center gap-2 ${itemGridPositions[index]}`}
      >
        <div className='flex h-16 w-16 items-center justify-center'>
          <img
            src={item.isAchieved ? itemIcons[item.iconKey] : unlockIcon}
            alt={item.isAchieved ? `${item.name} 아이콘` : `${item.name} 잠김`}
        ...
        <span
          className={`text-sm font-medium ${
            item.isAchieved ? 'text-main-green2' : 'text-text'
          }`}
        >
          {item.name}
        </span>
...
<div className='px-5 pb-8 pt-4 text-center'>
  <p className='text-base font-semibold leading-6 text-text'>
    다음 물품까지{' '}
    <span className='font-bold text-main-green2'>{remainingCount}번</span>
    {' '}남았어요!
  </p>
