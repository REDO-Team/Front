...
import { MY_CONTRIBUTION } from '../mocks/contribution';
...
export default function MyContributionPage() {
  ...
  const { nickname, recycleCount, achievedItemName, remainingCount, items } = MY_CONTRIBUTION;
    ...
    <div className='min-h-screen bg-[#F4FFF8] pt-14'>
      <section>
        <div className='px-5 py-6'>
          <h2 className='text-[20px] font-bold leading-[1.35] text-text'>
            {nickname}님,
            <br />
            지금까지 {recycleCount}번의 분리수거로
            <br />
            <span className='text-main-green2'>{achievedItemName}</span>를 만들었어요!
          </h2>
        </div>
      </section>
        ...
        <div className='relative'>
          ...
          <div className='relative z-10 grid grid-cols-6 gap-y-7'>
            {items.map((item, index) => (
              <div key={item.id} className={`col-span-2 flex flex-col items-center gap-2 ${itemGridPositions[index]}`}>
                <div className='flex h-16 w-16 items-center justify-center'>
                  <img src={item.isAchieved ? itemIcons[item.iconKey] : unlockIcon} alt={item.isAchieved ? `${item.name} 아이콘` : `${item.name} 잠김`} className='h-full w-full' />
                </div>

                <span className={`text-sm font-medium ${item.isAchieved ? 'text-main-green2' : 'text-text'}`}>{item.name}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className='px-5 pb-8 pt-4 text-center'>
        <p className='text-base font-semibold leading-6 text-text'>
          다음 물품까지 <span className='font-bold text-main-green2'>{remainingCount}번</span> 남았어요!
        </p>
      ...
      <button type='button' onClick={() => navigate('/all-contribution')} className='mx-5 mb-8 flex h-12 w-full items-center justify-center rounded-full bg-main-green1 text-base font-bold text-white shadow-[0_5px_12px_rgba(0,0,0,0.08)]'>
        <p>함께 만드는 변화 보러가기</p>
      </button>
    </div>
  );
