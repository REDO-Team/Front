...
export default function RewardAddressSearchPage() {
  ...
  const handleSearch = (event: FormEvent<HTMLFormElement>) => {
    ...
    setAddressCandidates(
      MOCK_ADDRESS_SEARCH_RESPONSE.result?.addressCandidates ?? [],
    );
    setHasSearched(true);
  };
      ...
      <form className='mt-4 flex items-center gap-3' onSubmit={handleSearch}>
        <label className='flex h-16 min-w-0 flex-1 items-center rounded-[28px] bg-white px-5'>
          ...
          <input
            ...
            className='ml-3 min-w-0 flex-1 bg-transparent text-base font-semibold text-text outline-none placeholder:text-gray-500'
          />
        </label>
        <button
          type='submit'
          className='h-16 w-[86px] shrink-0 rounded-[28px] bg-main-green1 text-lg font-bold text-white'
        >
          검색
        </button>
      </form>
      ...
      {hasSearched ? (
        <section className='mt-6' aria-live='polite'>
          <h2 className='px-1 text-lg font-bold text-text'>
            주소 검색 결과
            <span className='ml-2 text-main-green1'>
          ...
          {addressCandidates.length > 0 ? (
              ...
              {addressCandidates.map((address) => (
                ...
                >
                  ...
                  <div className='mt-5 flex items-start gap-3'>
                    <span className='flex h-10 w-[92px] shrink-0 items-center justify-center rounded-full bg-bg-green2 text-sm font-semibold text-gray-700'>
                      지번
                    </span>
                    <p className='min-w-0 pt-1 text-base font-semibold leading-[1.6] text-gray-500'>
            ...
            <div className='mt-3 rounded-[22px] bg-white px-5 py-10 text-center text-sm font-semibold text-gray-500'>
              검색 결과가 없습니다.
            </div>
          )}
        </section>
        ...
        <section className='mt-6 rounded-[28px] bg-bg-green2 px-6 py-7 text-gray-700'>
          ...
          <div className='mt-5'>
            <p className='flex items-center gap-2 text-base font-semibold leading-none'>
              <span
                className='h-2 w-2 shrink-0 rounded-full bg-main-green1'
                aria-hidden='true'
              />
              도로명 + 건물번호
            </p>
