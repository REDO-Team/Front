    ...
    <div className='bg-bg-green1 min-h-screen pb-24 relative font-pretendard'>
      ...
      <main className='px-5 pt-4 pb-6'>
        <div className='flex items-center gap-1.5 mb-3'>
          <span className={`inline-flex items-center justify-center px-[9px] py-[4px] rounded-[20px] text-[11px] font-bold leading-none ${getCategoryStyle(formatCategory(post.category))}`}>{formatCategory(post.category)}</span>
          {isMyPost && <span className='inline-flex items-center justify-center px-[9px] py-[4px] rounded-[20px] text-[11px] font-bold leading-none text-white bg-main-green1'>내 글</span>}
        </div>

        {/* 게시글 제목 */}
        <h1 className='text-[22px] font-bold text-gray-900 leading-snug break-keep mb-4'>{post.title}</h1>
        ...
        <div className='bg-white rounded-[20px] p-5 shadow-[0_4px_10px_rgba(0,0,0,0.03)] mb-6'>
          <p className='text-[16px] font-medium text-gray-900 leading-relaxed whitespace-pre-wrap break-keep mb-6'>{post.content}</p>
          {post.imageUrls && post.imageUrls.length > 0 && (
            <div className='flex gap-3 mt-4 overflow-x-auto pb-2 scrollbar-hide'>
              {post.imageUrls.map((url: string, index: number) => (
                <img key={index} src={url} alt={`게시글 이미지 ${index + 1}`} className='w-[140px] h-[140px] rounded-[10px] object-cover shrink-0' onClick={() => setSelectedImage(url)} />
              ))}
            </div>
          )}
        </div>
        ...
        <div className='flex flex-row items-center gap-3 whitespace-nowrap text-[14px] font-semibold leading-[22px] text-gray-500 mb-4'>
          ...
          <button className='flex items-center gap-1 text-gray-900' onClick={handleLikeToggle}>
            <HeartIcon className={`w-[17px] h-[15px] ${!isLiked && 'text-gray-500'}`} isFilled={isLiked} />
        ...
        <section className='flex flex-col gap-3'>
          ...
          {comments.map((comment, index) => (
            <div key={comment.commentId} className='flex flex-col'>
              ...
              {index !== comments.length - 1 && <hr className='border-t-[1px] border-gray-300 mt-4' />}
            </div>
          ))}
        </section>
      </main>
      ...
      <footer className='fixed bottom-0 left-0 right-0 mx-auto w-full max-w-120 bg-white rounded-t-[30px] shadow-[0_-4px_10px_rgba(0,0,0,0.05)] px-[18px] py-[16px] pb-[30px]'>
        ...
        <div className='flex items-center gap-[10px]'>
          ...
          />
          <button onClick={handlePostComment} className='bg-main-green1 text-white text-[14px] font-bold px-5 py-2.5 rounded-full shrink-0'>
            게시
          </button>
        </div>
      </footer>
      ...
      {selectedImage && (
        <div className='fixed inset-0 z-[100] flex items-center justify-center bg-black/80 px-4' onClick={() => setSelectedImage(null)}>
          <button className='absolute top-6 right-6 text-white text-[30px] font-bold' onClick={() => setSelectedImage(null)}>
            &times;
          </button>

          <img src={selectedImage} alt='확대 이미지' className='max-w-full max-h-[80vh] rounded-[10px] object-contain' onClick={(e) => e.stopPropagation()} />
        </div>
      )}
    </div>
  );
