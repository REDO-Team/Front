    ...
    <header className={`${position} top-0 w-full max-w-120 flex items-center justify-between h-14 px-4 z-50 ${bgColor}  ${!leftIcon && !leftText && !title ? 'justify-end' : 'justify-between'}`}>
      ...
      {(leftIcon || leftText) && (
        <div className='flex items-center justify-center z-10 text-lg font-semibold leading-none text-text' onClick={handleBack}>
          ...
          {leftText && (
            <span className='text-lg font-semibold leading-none text-center text-black' style={{ color }}>
              {leftText}
            </span>
          )}
        </div>
      ...
      {title && (
        <div className='absolute inset-0 flex items-center justify-center px-12 pointer-events-none'>
          <span className='max-w-full truncate whitespace-nowrap text-lg font-pretendard font-semibold leading-none text-center text-text' style={{ color }}>
            {title}
          </span>
        </div>
      )}
      ...
      {rightIcon && (
        <button type='button' onClick={onClick}>
          {typeof rightIcon === 'string' ? <img src={rightIcon} alt='홈으로 가기' className='ml-auto flex items-center justify-end z-10 text-base font-semibold text-text ' /> : rightIcon}
        </button>
      )}
    </header>
  );
