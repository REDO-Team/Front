...
export default function RecycleCategoryCard({ padding, gap, radius, imgSize, categorySize, nameSize, content, category = '플라스틱', name = '투명 페트병' }: RecycleCategoryCardProps) {
  const image = CATEGORY_LIST.filter((cl) => cl.category === category)[0].image;
  ...
  return (
    <div className={`${name.includes('쓰레기') ? 'flex bg-gray-400' : 'flex bg-linear-to-r from-main-green1 to-main-sky rounded-xl shadow-md shadow-main-green1/15'}`} style={{ padding, gap, borderRadius: radius }}>
      {name ? <img src={image} className='object-fill' style={{ width: imgSize, height: imgSize, borderRadius: radius }} /> : <div className={`opacity-45 bg-white`} style={{ width: imgSize, height: imgSize, borderRadius: radius }}></div>}
